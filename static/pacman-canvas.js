...
var quotes = {
	"osborne" : [
		"Let's discuss this over canopies...",
		"That's what sweetheart deals are for!",
		"Let's keep this between you and I..."
	],
	"google" : [
		"Mmm... tax free moolah",
		"UK profits; Bermuda taxes",
		"Death and taxes? I think not!",
		"Hospitals are overrated!",
		"Funnel that through the Irish 'HQ'",
		"Wahoo! Where's the nearest tax haven?",
		"3% tax is more than enough",
		"Who needs teachers when you have cashola?",
		"That's a few grand less for the NHS",
		"Who needs teachers when you have profits?"
	],
	"special" : [
		"3% tax rate work for you?",
		"Mwhahahahahahaha!",
		"Feel my sweetheart deals",
	...
	"share" : [
		"DO NOT tell your friends about our arrangement!"
	]
}
...
var caughtNumber = 0;
     ...
     function initialSignup(){
        ...
        ga('send', 'event', 'petition-signup', 'clicked');


        console.log($('#initial-signup').serialize());

        $.post($('#initial-signup').attr('action'), $('#initial-signup').serialize(), function(response) { 
        ...
        $.ajax({
          async: true,
		  method: "POST",
		  url: $('#initial-signup').attr('action'),
		  data: $('#initial-signup').serialize()
		})
		  .done(function( msg ) {
		...
		*/
        ...
        //$(window).scrollTo($(nextSlide), 800);
          ...
          $('#donation-slide').show();
          $(window).scrollTo($('#donation-slide'), 800);
     ...
     function gameSignup(){
        ...
        ga('send', 'event', 'game-signup', 'clicked');


        //alert($('#game-signup').serialize());

        $.post($('#game-signup').attr('action'), $('#game-signup').serialize(), function(response) { 

        });

	    $('.email:eq(1)').val($('.email:eq(0)').val());
	    $('.firstname:eq(1)').val($('.firstname:eq(0)').val());    
        //facebookSignin();
        ...
        //$(window).scrollTo($(nextSlide), 800);
          ...
          $('.slide:eq(1)').show();
          $(window).scrollTo($('.slide:eq(1)'), 800);

        // disable submit button to prevent multiple submits
        jQuery('#game-signup input[type=submit]').prop('disabled', true).addClass('disabled');
        console.log('disable submit');
        setTimeout(function(){  // just for demo
            console.log('enable submit');
            jQuery('#game-signup input[type=submit]').prop('disabled', false).removeClass('disabled');
        },1500);
...
/* ----- Global Variables ---------------------------------------- */
	...
	var mapConfig = "static/data/map.json";
	...
	function ajax_add(n, s, l) {
		...
		$.ajax({
		   type: 'POST',
		   url: 'static/data/db-handler.php',
		   data: {
			 action: 'add',
			 name: n,
			 score: s,
	...
	function Game() {
		...
		//this.ghostSpeedNormal = (this.level > 4 ? 3 : 3);	// global default for ghost speed
		...
		this.drawHearts = function (count) {
			...
			for (var i = 0; i<count; i++) {
				html += " <img src='static/img/heart.png'>";
				}
			$(".lives").html("Sweetheart deals: "+html);
		...
		this.init = function (state) {
			...
			if( state === 0 ) {
                this.timer.reset();
                $('.tax').html("Tax: &pound;0");
                this.taxes = 0;
                gameOverNow = false;
                $('.bored-yet.nosm .next').show();
                $('.bored-yet .button-container').hide();
                $('.bored-yet.nosm .skip').remove();
                $('.bored-yet.nosm .fb-share').remove(); 
			}
			...
			if (pinky === null || pinky === undefined) {
				pinky = new Ghost("pinky",7,5,'static/img/osborne2.png',2,2);
				inky = new Ghost("inky",8,5,'static/img/osborne2.png',13,11);
				blinky = new Ghost("blinky",9,5,'static/img/osborne2.png',13,0);
				clyde = new Ghost("clyde",10,5,'static/img/osborne2.png',2,11);
			}
			else {
	...
	function Taxes(tax) {
		$('.tax').html("Tax: &pound;"+numberWithCommas(tax));
	}
	...
	function Score() {
		...
		this.refresh = function(h) {
			$(h).html("Profits: &pound;"+numberWithCommas(this.score));
		};
	...
	function Ghost(name, gridPosX, gridPosY, image, gridBaseX, gridBaseY) {
		...
		this.dazzleImg.src = 'static/img/osborne-scared.png';
		this.dazzleImg2 = new Image();
		this.dazzleImg2.src = 'static/img/osborne-done.png';
		this.deadImg = new Image();
		this.deadImg.src = 'static/img/dead.svg';
		...
		this.move = function() {
			...
			if (this.ghostHouse == true) {
				...
				if (this.name == "clyde") {
					if ((game.level < 1) || ((game.pillCount > 104/3))) this.stop = true;
					else this.stop = false;
				}
				// Inky starts after 30 pills and only from the third level on
				if (this.name == "inky") {
					if ((game.level < 1) || ((game.pillCount > 104-30))) this.stop = true;
					else this.stop = false;
				}
	...
	function pacman() {
		...
		this.dieFinal = function() {
			...
			game.taxes = Math.round(parseInt(game.score.score)*0.03);
			Taxes(game.taxes);
			this.reset();
			pinky.reset();
			inky.reset();
	    	...
	    	if (this.lives <= 0) {
				...
				var taxrate = Math.round((game.taxes/game.score.score)*100);
				var shareHref = "http://taxman.sumofus.org/"+numberWithCommas(game.score.score)+"/";
				game.showMessage("Good work!","You made a profit of &pound;"+numberWithCommas(game.score.score)+" and were taxed &pound;"+numberWithCommas(game.taxes)+"<br /><br />That's a "+taxrate+"% tax rate - Google would be proud!<br /><br /><a href='javascript:;' data-href='https://www.facebook.com/sharer/sharer.php?u="+shareHref+"' class='button fb-share'>Share on Facebook</a><br /><br /><a href='javascript:;' class='skip'>Skip</a>");
				game.gameOver = true;
				$('#playerName').focus();
				$('.next').hide();
				$('.bored-yet.nosm .next').after("<a href='javascript:;' data-href='https://www.facebook.com/sharer/sharer.php?u="+shareHref+"' class='button fb-share'>Share on Facebook</a><br /><br /><a href='javascript:;' class='skip'>Skip</a>");
				gameOverNow = true;
				quote("share");
				} else {
					
					if(this.lives == 3){
						game.showMessage("You got caught avoiding your taxes!","Not to worry - we like you, so we'll only charge you a small fee.<br /></br />It's what's commonly referred to as a Sweetheart Deal.<br /><br /><a href='javascript:;' class='buttons'>Carry on making money</a>");
					} else if (this.lives == 2){
						game.showMessage("Time to pay up!","We'll cut you a sweetheart deal - let's call it 3%<br /><br /><a href='javascript:;' class='buttons'>Carry on making money</a>");
					} else {
						game.showMessage("You got caught avoiding your taxes!","You'll be charged a small fee in taxes.<br /><br /><a href='javascript:;' class='buttons'>Carry on making money</a>");
					}
...
function checkAppCache() {
	...
	$(document).on('click', '.skip', function(event){
		ga('send', 'event', 'skip-share', 'clicked'); 
		...
		$(document).on('click', '.fb-share', function(event){
     	ga('send', 'event', 'fb-share', 'clicked'); 
    ...
    person['variation'] = getParameterByName('variation');
    person['akid'] = getParameterByName('akid');


        // prefill personalised fields with passed-in URL parameters

        if (person['firstname']) { $('.firstname').val(person['firstname']); }
        if (person['email']) { $('.email').val(person['email']); }
        if (person['zip']) { $('.zip').val(person['zip']); }
        if (person['source']) { $('.source').val(person['source']); }
        if (person['sub']) { $('.sub').val(person['sub']); }
        if (person['variation']) { $('.variation').val(person['variation']); }   
        if (person['akid']) { $('.akid').val(person['akid']); }    
...
/*
    $("#game-signup .form-control").change(function(){
      validateSignupForm(this);   
    });
*/

    $("#initial-signup .form-control").not('#country').change(function(){
      validateSignupForm(this);   
    });
      ...
      if (person['firstname'] != "" && person['email'] != "") {
		...
		function renderContent()
		{
			...
			$.each(game.map.posY, function(i, item) {
			   ...
			   $.each(this.posX, function() { 
				   if (this.type == "pill") {
					...
					context.drawImage(googleImg,pacman.posX,pacman.posY,30,30);
					...
					*/
					...
					context.moveTo(game.toPixelPos(this.col-1), game.toPixelPos(dotPosY-1));
				   	...
				   	var coinImg=document.getElementById("coin-image");
				   	context.drawImage(coinImg,game.toPixelPos(this.col-1)+7,game.toPixelPos(dotPosY-1)+7,game.powerpillSizeCurrent*2,game.powerpillSizeCurrent*2);
			...
			if (game.running == true) {
				...
				// Pac Man
				//add google image
				var googleImg=document.getElementById("google-image");
				//var pattern1=context.createPattern(googleImg,"no-repeat");
	...
	function doKeyDown(evt){
		console.log("GON "+gameOverNow);
	if(gameOverNow == false && onFirstSlide == false){
		switch (evt.keyCode)
			{
    ...
    //manage quotes popping up
    var quoteTime;
    var quoteGoogleCycle = 0;
    var quoteOsborneCycle = 0;
    var quoteSpecialCycle = 0;
    var quoteLevelCycle = 0;
    var changingQuote = false;
    var isGoogleQuote = false;
    var isOsborneQuote = false;
    var buttonShown = false;
    var gameOverNow = false;
    var onFirstSlide = true;
    function quote(who){
	   ...
	   // 1. WHENEVER EATING  - show google TICK
	   // 2. Whenever GO is caught - so GO TICK 
	   // 3. WHENEVR USER IS CAUGHT - Show GO2
	   // 4. WHENEVR PAUSED - DELETE THE IMAGE TICK
	   if(winWidth > 768){
    	...
    	if(who == "osborne"){	
    		...
    		if(isOsborneQuote == true){

    		$('#quote-bubble').fadeOut(function() {	
    			$('#quote-bubble').html(quotes["osborne"][quoteOsborneCycle]);	
    			$('#quote-bubble').fadeIn();
    		});	
    		...
    		$('#quote-container').fadeOut(function() {	
    			$('#quote-container').html("<div class='triangle-right' id='quote-bubble'>"+quotes["osborne"][quoteOsborneCycle]+"</div><img src='/static/img/osborne2.png' id='quote-image'>");	
	    	...
	    	quoteOsborneCycle++;
		    if(quoteOsborneCycle == quotes["osborne"].length){
		      quoteOsborneCycle= 0;
		    }

    		quoteTime = setTimeout(function(){ changingQuote = false;  }, 2500);

    	} else if (who == "google") {
    		
    			if(changingQuote == false){

    			//make sure there isn't already a quote up there	
	    			changingQuote = true;
	    			
	    			//if its a google quote then just change the quote

	    			if(isGoogleQuote == true){
	    				$('#quote-bubble').fadeOut(function() {	
	    					$('#quote-bubble').html(quotes["google"][quoteGoogleCycle]);
	    					$('#quote-bubble').fadeIn();
	    				});
	    			} else {
	    				$('#quote-container').fadeOut(function() {	
				    		$('#quote-container').html("<div class='triangle-right' id='quote-bubble'>"+quotes["google"][quoteGoogleCycle]+"</div><img src='/static/img/google2.png' id='quote-image'>");	
				    		$('#quote-container').fadeIn();
				    	});
	    			}

	    			isGoogleQuote = true;
	    			isOsborneQuote = false;

	    			quoteGoogleCycle++;

				    if(quoteGoogleCycle == quotes["google"].length){
				      quoteGoogleCycle= 0;
				    }
    		...
    		if(isOsborneQuote == true){
	    		...
	    		$('#quote-bubble').fadeOut(function() {	
	    			$('#quote-bubble').html(quotes["special"][quoteSpecialCycle]);	
	    			$('#quote-bubble').fadeIn();
	    		});	
	    		...
	    		$('#quote-container').fadeOut(function() {	
	    			$('#quote-container').html("<div class='triangle-right' id='quote-bubble'>"+quotes["special"][quoteSpecialCycle]+"</div><img src='/static/img/osborne2.png' id='quote-image'>");	
	    			$('#quote-container').fadeIn();
	    		});	
	    	...
	    	quoteSpecialCycle++;
		    if(quoteSpecialCycle == quotes["special"].length){
		      quoteSpecialCycle= 0;
		    }
    		...
    		if(isOsborneQuote == true){
	    		...
	    		$('#quote-bubble').fadeOut(function() {	
	    			$('#quote-bubble').html(quotes["share"][0]);	
	    			$('#quote-bubble').fadeIn();
	    		});	
	    		...
	    		$('#quote-container').fadeOut(function() {	
	    			$('#quote-container').html("<div class='triangle-right' id='quote-bubble'>"+quotes["share"][0]+"</div><img src='/static/img/osborne2.png' id='quote-image'>");	
	    			$('#quote-container').fadeIn();
	    		});	
    		...
    		if(isOsborneQuote == true){
	    		...
	    		$('#quote-bubble').fadeOut(function() {	
	    			$('#quote-bubble').html(quotes["level"][quoteLevelCycle]);	
	    			$('#quote-bubble').fadeIn();
	    		});	
	    		...
	    		$('#quote-container').fadeOut(function() {	
	    			$('#quote-container').html("<div class='triangle-right' id='quote-bubble'>"+quotes["level"][quoteLevelCycle]+"</div><img src='/static/img/osborne2.png' id='quote-image'>");	
	    			$('#quote-container').fadeIn();
	    		});	
	    	...
	    	quoteLevelCycle++;
		    if(quoteLevelCycle == quotes["level"].length){
		      quoteLevelCycle= 0;
		    }
    ...
    function slidingFunction(stage){
    ...
    function showButton(){
    	if(buttonShown == false){
    		buttonShown = true;
    		setTimeout(function(){ $('.bored-yet .button-container').show(); }, 2000);
    	}
   ...
   function validateSignupForm(el){
      console.log("CHANGE");
      var result = $("#initial-signup").validate().element(el);
      var span = $(el).next('label');
      $(el).closest('.form-group').find('.glyphicon').remove();
      //console.log(result);
      if(result == false){    
          $(el).after('<span class="glyphicon glyphicon-remove form-control-feedback">');
